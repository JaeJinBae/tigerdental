...
//사이트관리
function site_it($mode, $param){
	var fm = document.site,
		$cnt = $("#site input:checkbox:checked").not("#selectall").size(),
		answer = "";

	if($mode == "insert_form"){
		location.href = "site_form.html?mode=insert&" + $param;

	}else if($mode == "list"){
		location.href = "site_list.html?" + $param;

	}else if($mode == "reset"){
		fm.reset();
		answer = confirm("리스트페이지로 이동하시겠습니까?");
		if(answer)	site_it('list', $param);
		...
		answer = confirm("삭제하시겠습니까?");
		if(answer)	location.href = "site_proc.php?mode="+ $mode +"&" + $param;
...
//서병수 공략집 관리
function seo_scheme_it($mode, $param){
	var fm = document.seo_scheme,
		$cnt = $("#seo_scheme input:checkbox:checked").not("#selectall").size(),
		answer = "";

	if($mode == "insert_form"){
		location.href = "seo_scheme_form.html?mode=insert&" + $param;

	}else if($mode == "list"){
		location.href = "seo_scheme_list.html?" + $param;

	}else if($mode == "reset"){
		fm.reset();
		answer = confirm("리스트페이지로 이동하시겠습니까?");
		if(answer)	seo_scheme_it('list', $param);
		...
		answer = confirm("삭제하시겠습니까?");
		if(answer)	location.href = "seo_scheme_proc.php?mode="+ $mode +"&" + $param;

	}else if($mode == "submit"){
		if( !$("#seo_scheme").validate() ) return;
		...
		if($total <= 0){
			...
			instance_iframe("seo_scheme_excel.php?mode="+$mode);
		}
		...
		if($total <= 0){
			...
			$("#seo_scheme input:checkbox:checked").not("#selectall").each(function(){
				$seq_list += $(this).val() + ",";
			});

			$seq_list = $seq_list.replace(/,$/g, '');

			instance_iframe("seo_scheme_excel.php?"+$param+"&seq_list="+$seq_list+"&cnt="+$cnt+"&mode="+$mode);
		}

	}else if($mode == "download"){
		instance_iframe("seo_scheme_download.php?seq="+$param);

	}else if($mode == "attach_del"){
		answer = confirm("삭제하시겠습니까?");

		if(answer){
			$.post("seo_scheme_proc.php", {mode : $mode, seq : $param}, function(data){
				if(data == 0){
					alert("오류가 발생하였습니다");
					return;
				}else{
