...
$(function(){
	...
	$(".left_menu > dl:nth-child(2) > dd:nth-child(2) > a").addClass("on");
	//게시판 검색
									...
									<c:if test="${fn:length(topList) != 0}">
										<c:forEach var="item" items="${topList}">
											<tr class="cont">
												...
												<td>
													<a href="${pageContext.request.contextPath}/admin/menu02_01update${pageMaker.makeSearch(pageMaker.cri.page)}&no=${item.no}"><p class="title"> ${item.title}</p></a>
												</td>
												<td>${item.writer}</td>
												<td>${item.regdate}</td>
												<td></td>
