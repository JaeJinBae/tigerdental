			...
			<div class="main_bottom_area">
				<div class="list_area">
									...
									<c:if test="${fn:length(topList) != 0}">
										<c:forEach var="item" items="${topList}">
											<tr class="cont">
												...
												<td>
													<a href="${pageContext.request.contextPath}/admin/menu02_02update${pageMaker.makeSearch(pageMaker.cri.page)}&no=${item.no}"><p class="title"> ${item.title}</p></a>
												</td>
												<td>${item.writer}</td>
												<td>${item.regdate}</td>
												<td></td>
										...
										<c:set var="num" value="${pageMaker.totalCount - ((pageMaker.cri.page -1) *10)}"></c:set>
									        <c:forEach var="item" items="${list}">
												<tr class="cont">
													...
													<td><a href="${pageContext.request.contextPath}/admin/menu02_02update${pageMaker.makeSearch(pageMaker.cri.page)}&no=${item.no}"><p class="title"> ${item.title}</p></a></td>
													<td>${item.writer}</td>
													<td>${item.regdate}</td>
													<td></td>
													<td>${item.cnt}</td>
					...
					<div class="btn_area">
						...
						<p class="btn_right">
							<button type="button" class="btn_black" onclick="location.href='${pageContext.request.contextPath}/admin/menu02_02register'">등록</button>
						</p>
					</div>
