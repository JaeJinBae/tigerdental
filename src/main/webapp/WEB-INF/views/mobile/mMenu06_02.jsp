				...
				<c:if test="${pageMaker.cri.page == 1}">
					<c:if test="${fn:length(topList) != 0}">
						<c:forEach var="item" items="${topList}">
							<li>
								<a href="${pageContext.request.contextPath}/m/menu06_02read${pageMaker.makeSearch(pageMaker.cri.page)}&no=${item.no}">
									<b>  ${item.title}</b>
									<!--i class='name'>관</i><span class='line'>|</span--><i class="date">${item.regdate}</i>
								</a>	
							</li>
						</c:forEach>
					</c:if>
